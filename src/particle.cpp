...
// simulate double stim with variable inter spike interval in milliseconds
// The name is misleading because the frequency can be set through the constants.
void Trajectory::simulate_doubles_3kHz(gsl_rng* rng, const param& par, const constpar *constants, double isi){
    ...
    for(unsigned int t=1;t<size;t++){
        burst(t) = (t*dt>=0.01 && t*dt<0.03) ? 1 : 0;
...
void SMC::sampleParameters(const param &pin, param &pout, Trajectory &traj, reparam *tau2gamma){
    ...
    sampled_variable = gsl_rng_uniform_int(rng,4);
    //do {
    //    sampled_variable = gsl_rng_uniform_int(rng,4);
    //} while (sampled_variable==1);
    ...
    // Force Amax not to go below 2sd from the prior mean. This is equivalent to use a truncated gaussian as prior on Amax. 
    bool Amax_good = Amax_test>constants->Amax_prior_mean-constants->Amax_prior_sd;
    //bool Amax_good = true;

    if(gsl_rng_uniform(rng) < exp(log_alpha_MH) && Amax_good) {
        pout.Amax  = Amax_test;
        pout.A     = A_test;
        pout.c0    = c0_test;
